          ...
          fluidRow(
            bs4Dash::box(width = 12,status = "success",title = "Descriptive",solidHeader = TRUE,
                   ...
                   awesomeCheckbox(inputId = ns("factor_hist"),
                                   ...
                                   value = F,
                                   status = "danger"),
                   selectInput(inputId = ns("Id088f"), label = "Factor",choices = "",width =  '100%' ),
                   sliderInput(ns("alphagD"), label = "Alpha Density", min = 0, max =  1, value =  0.2 ,width = "100%" ),
                   awesomeCheckbox(inputId = ns("Facewrap"),
                 ...
                 tabPanel(
                   ...
                   dropdown(
                     prettyRadioButtons(inputId = ns("typefileDes"),label = "Download Plot File Type", outline = TRUE,fill = FALSE,shape = "square",inline = TRUE,
                                        choices = list(PNG="png",PDF="pdf"),icon = icon("check"),animation = "tada" ),
                     conditionalPanel(condition="input.typefileDes=='png'", ns = ns,
                                      sliderInput(inputId=ns("png.wid.d"),min = 200,max = 2000,value = 900,label = "Width pixels") ,
                                      sliderInput(inputId=ns("png.hei.d"),min = 200,max = 2000,value = 600,label = "Height pixels")
                                      ),
                     conditionalPanel(condition="input.typefileDes=='pdf'", ns = ns,
                                      sliderInput(inputId=ns("pdf.wid.d"),min = 2,max = 20,value = 10,label = "Width") ,
                                      sliderInput(inputId=ns("pdf.hei.d"),min = 2,max = 20,value = 8,label = "Height")
                                      ),
                     
                     downloadButton(ns("descargar.d"),
                                    "Download plot",class = "btn-success",
                   ...
                   plotOutput(ns("plothistogram"),
                              ...
                              brush = brushOpts(
                                id = ns("plot1_brush"),direction = "x", fill = "#ccc"
                              )),
                   shinyjs::hidden(
                     div(id=ns("sel"),
                         hr(),
                         shinycssloaders::withSpinner( 
                           DT::dataTableOutput(ns("dataSele")),type = 5,color = "#28a745" 
                         )
                     )
                   )
                 ),
...
mod_distribution_server <- function(input, output, session, data){
  ...
  observeEvent(data$data(), {
    updateSelectInput(session, "Id088", choices=names(data$data()),selected = "YdHa_clean")
    updateSelectInput(session, "Id088f", choices=names(data$data()),selected = "NNNNN")
  })
  
  observe({
    toggle(id = "Id088f", condition = input$factor_hist, animType = "fade",anim = TRUE)
  })
  
  observe({
    toggle(id = "Facewrap", condition = input$factor_hist, animType = "fade",anim = TRUE)
  })
  
  observe({
    toggle(id = "alphagD", condition = input$factor_hist, animType = "fade",anim = TRUE)
  })
    ...
    if(is.numeric(dt[ , input$Id088])){
      if((input$Id088f)==""|!isTRUE(input$factor_hist)){
        ggplot(dt, aes_string(x=input$Id088))+
          geom_histogram(aes(y=..density..), alpha=input$alphagg, na.rm=TRUE, bins = input$bindwidth)+
          theme_bw()+
          geom_density(alpha=input$alphagD,  na.rm=TRUE) 
      } else{ 
        dt[,input$Id088f] <- as.factor(dt[,input$Id088f])
        p <-  ggplot(dt, aes_string(x=input$Id088,fill=input$Id088f, color=input$Id088f))+
          geom_histogram(aes(y=..density..), alpha=input$alphagg,position="identity",  na.rm=TRUE, bins = input$bindwidth)+
          theme_bw()+
          geom_density(alpha=input$alphagD,  na.rm=TRUE)
      ...
      ggplot(dt   , aes_string(x=input$Id088))+geom_bar( na.rm=TRUE)+theme_bw()+coord_flip()
    }
  })
  ...
  output$descargar.d <- downloadHandler(
    ...
    content = function(file){
      ...
      if(is.numeric(dt[ , input$Id088])){
        ...
        p <- ggplot(dt   , aes_string(x=input$Id088))+geom_bar( na.rm=TRUE)+theme_bw()+coord_flip()
      }
  ...
  infoselect <- reactive({
    ...
    data$data() %>% 
      dplyr::filter(.data[[res$mapping$x]]>= res$xmin & .data[[res$mapping$x]]<= res$xmax) 
  })
  
  output$dataSele <- DT::renderDataTable({
    req(input$plot1_brush)
    req(infoselect())
    DT::datatable(infoselect(),
                  extensions = 'Buttons', filter = 'top', selection="multiple",
                  options = list(dom = 'lfrtipB', scrollX = TRUE, pageLength = 2,lengthMenu = c(1:4,nrow(infoselect())),
                                 columnnDefs=list(list(className = 'dt-center', targets = 0:ncol(infoselect()))) ))
  })
  

  observe({
    toggle(id = "sel", condition = !is.null(input$plot1_brush), animType = "fade",anim = TRUE)
  })
