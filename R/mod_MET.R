...
mod_MET_server <- function(input, output, session){
    ...
    tryCatch(
      {
        ...
        if(!Ext%in%c("csv","CSV")) return()  # if(Ext!="csv") 
  ...
  datafilter <- reactive({
    ...
    input$file1
    req(dataset())
    if(!input$genotype %in% names(dataset())) return()
    if(!input$experiment %in% names(dataset())) return()
    if(!input$variable %in% names(dataset())) return()
    if(!input$weight %in% names(dataset())) return()
    
    tmp_data <- dataset()
    if(!is.null(input$selected)) tmp_data <- dataset() %>% dplyr::filter(.data[[input$experiment]] %in% input$selected) %>% droplevels()
    ...
    req(datafilter()$data)
      if(input$visual=="Number"){
        ...
        df <- datafilter()$data %>%
              ...
              dplyr::summarise(mean = round(mean(.data[[datafilter()$resp]], na.rm = T),2), 
