...
mod_asreml_selector_server <- function(input, output, session, data){
  ...
  modelo <- reactive({
    ...
    isolate({
      ...
      tryCatch(
        { 
         model <-spatial.selector(data = dt,
  ...
  output$models <- DT::renderDataTable({
    ...
    isolate({
    req(modelo())
    dt <- modelo()$parms %>% dplyr::mutate_if(is.numeric, round, 3) %>% dplyr::mutate(MODEL=as.factor(MODEL))
    DT::datatable(dt,
                  extensions = 'Buttons', filter = 'top', selection="multiple",
                  options = list(dom = 'lfrtip', scrollX = TRUE, pageLength = 10,lengthMenu = c(5,10,15,20,33),
  ...
  output$plot <- plotly::renderPlotly({
    ...
    req(modelo())
      dt <- modelo()$parms
      g1 <- dt %>% ggplot(aes(x=MODEL, y= .data[[input$criteria]]))+geom_line()+geom_point()+theme_bw()
      plotly::ggplotly(g1)
    })
  })
