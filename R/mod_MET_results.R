                     ...
                     bs4TabCard(width = 12,id = "MET_as",maximizable = T,solidHeader = FALSE,closable = F,
                                ...
                                tabPanel(title = "Covariance Matrix",icon = icon("signal"),
                                            fluidRow(
                                              col_3(
                                                dropdown(
                                                  prettyRadioButtons(inputId = ns("filetype2"),label = "Download Plot File Type", outline = TRUE,fill = FALSE,shape = "square",inline = TRUE,
                                                                     choices = list(PNG="png",PDF="pdf"),
                                                                     icon = icon("check"),animation = "tada" ),
                                                  conditionalPanel(condition="input.filetype2=='png'", ns = ns,
                                                  ...
                                                  conditionalPanel(condition="input.filetype2=='pdf'", ns = ns,
                       ...
                       fluidRow(
                         col_6(
                           pickerInput(
                             ...
                             options = list(
                               title = "Please select a model ...",size = 5), width = "100%"
                           ) 
                         ),
                         col_6(
                           pickerInput(
...
mod_MET_results_server <- function(input, output, session, model){
  ...
  output$summ <- renderPrint({
    ...
    isolate({
      ...
      summary(model)$varcomp %>%
        ...
        dplyr::mutate_if(is.numeric, round, 3) %>% dplyr::select(1:4)
    })
  })
  ...
  observeEvent(input$pca,{
    showModal(modalDialog(
      ...
      conditionalPanel(condition = "input.type=='ind'",  ns = ns,
                        fluidRow(
                          col_4(
                          ),
                          col_4(
      ...
      conditionalPanel(condition = "input.type=='var'",  ns = ns,
                       fluidRow(
                         col_4(
                         ),
                         col_4(
      ...
      conditionalPanel(condition = "input.type=='bip'",  ns = ns,
                       fluidRow(
                         col_1(
                         ),
                         col_3(
  ...
  observeEvent(input$preds,{
    showModal(modalDialog(
      ...
      shinycssloaders::withSpinner(
        echarts4r::echarts4rOutput(ns("gxe")),type = 6,color = "#28a745"
        ) 
      )
    )
  }, ignoreInit = T, ignoreNULL = T)
