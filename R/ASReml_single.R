...
spatial.single <- function(data=NULL, gen=NULL, block=NULL, ibk=NULL, row=NULL, 
  ...
  if (type.gen=='fixed') { sols=NULL }
  if (type.gen=='random') { sols <- summary(mod.ref,coef=TRUE)$coef.random }
...
spatial.selector <- function(data=NULL, gen=NULL, block=NULL, ibk=NULL, row=NULL, col=NULL, 
  ...
  if (is.null(model)) { 
    ...
    for (i in 1:32) {
      ...
      if (type.residual=='ar1') {
        ...
        if (!nugget) { add.nugget <- mod.list$Nugget[i] }
      }
      mod.ref <-  spatial.single(data=data, gen=gen, block=block, ibk=ibk, row=row, 
    ...
    if (best.m>17 & best.m<32) { 
      ...
      gfit[33,2] <- gt$n.vc; gfit[33,3] <- gt$logL; gfit[33,4] <- gt$aic; gfit[33,5] <- gt$bic; 
      gfit[33,6] <- gt$herit.PEV; gfit[33,7] <- gt$herit.VC; gfit[33,8] <- gt$Aopt; gfit[33,9] <- gt$Dopt
      ...
      gfit[33,1] <- mod.list$Model[33]
      gfit[33,2] <- NA; gfit[33,3] <- NA; gfit[33,4] <- NA; gfit[33,5] <- NA; 
      gfit[33,6] <- NA; gfit[33,7] <- NA; gfit[33,8] <- NA; gfit[33,9] <- NA
    }
  ...
  if (!is.null(model)) { 
    ...
    mod <-  spatial.single(data=data, gen=gen, block=block, ibk=ibk, row=row, 
                           col=col,  cov1=cov1, cov2=cov2, resp=resp,
                           add.block=add.block,     add.ibk=add.ibk,  
                           add.row=add.row,         add.col=add.col,
                           add.spl.row=add.spl.row, add.spl.col=add.spl.col,
