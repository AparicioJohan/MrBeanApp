  ...
  output$plotblup2 <- plotly::renderPlotly({
    ...
    isolate({
      ...
      if ("check" %in% names(BLUPS)) {
        q <- ggplot2::ggplot(BLUPS, ggplot2::aes(x = gen, y = predicted.value, color = check))
      ...
      p <- q +
        ...
        ggplot2::geom_errorbar(ggplot2::aes(ymax = upper_95, ymin = lower_95)) +
        ggplot2::theme_bw() +
        ggplot2::geom_hline(yintercept = mean(BLUPS[, "predicted.value"]), linetype = 2, color = "grey") +
        theme(
          axis.title.x = ggplot2::element_blank(), 
          axis.text.x = ggplot2::element_blank(), 
          axis.ticks.x = ggplot2::element_blank()) +
        ggplot2::scale_x_discrete(limits = v) + 
        ggplot2::xlab("Genotypes")
