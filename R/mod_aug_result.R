             ...
             fluidRow(
               ...
               column(
                 ...
                 bs4Card(
                   ...
                   width = 12,title = "Histogram",status = "success",solidHeader = TRUE,collapsed = F)
                 ),
                 width = 6 
               )
             )
...
mod_aug_result_server <- function(input, output, session, model){
  ...
  BLUPS <- reactive({
    ...
    model$model()$predictions %>%
      dplyr::mutate(lower_95 = predicted.value - 1.645*std.error,
                    upper_95 = predicted.value + 1.645*std.error) %>% dplyr::select(-status)
  })
