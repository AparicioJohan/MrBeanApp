...
app_server <- function( input, output, session ) {
  ...
  observeEvent(input$toAwesome00, {updatebs4TabSetPanel(session = session, inputId = "tabs", selected = "home")})
  observeEvent(input$toAwesome11, {updatebs4TabSetPanel(session = session, inputId = "tabs", selected = "Data")})
  observeEvent(input$toAwesome22, {updatebs4TabSetPanel(session = session, inputId = "tabs", selected = "modelo")})
  observeEvent(input$toAwesome33, {updatebs4TabSetPanel(session = session, inputId = "tabs", selected = "valueboxes")})
