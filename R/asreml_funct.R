...
spatial.aud <- function(data=NULL, gen=NULL, check=NULL, block=NULL, row=NULL, col=NULL, 
  ...
  if (is.null(row) || is.null(col)) { 
    ...
    if (n != nr*nc) { stop('Number of observations does not conform with #rows x #columns.')}
  ...
  if (!is.null(check)) { 
    ...
    if (!is.null(check)) { 
      ...
      if (ncheck != 2) { stop('Number of levels in check should be 2: control / test') }
      lev_check <- unique(df$check)
      cont = 0; test = 0
      if (lev_check[1]=='control' | lev_check[2]=='control') { cont=1 }
      if (lev_check[1]=='test' | lev_check[2]=='test') { test=1 }
      if (cont+test != 2) { stop('Your levels for checks are not words: control / test')}
    }
  }
  if (!is.null(block)) { df$block <- as.factor(data[,block]) }
  if (!is.null(covariate)) { df$cov <- as.numeric(data[,covariate]) }
  if (is.null(resp)) { 
  ...
  if (nrand==0) { str.mod.ii <- paste(code.asr[1],code.asr[3],code.asr[4],sep=',') }
  if (nrand!=0) { str.mod.ii <- paste(code.asr[1],code.asr[2],code.asr[3],code.asr[4],sep=',') }
  ...
  if (nrand==0) { str.mod.ic <- paste(code.asr[1],code.asr[3],code.asr[4],sep=',') }
  if (nrand!=0) { str.mod.ic <- paste(code.asr[1],code.asr[2],code.asr[3],code.asr[4],sep=',') }
  # mod.ri
  code.asr[3] <- 'residual=~ar1(row):id(col)'
  code.asr[1] <- paste('mod.ri<-', code.t, sep='') 
  if (nrand==0) { str.mod.ri <- paste(code.asr[1],code.asr[3],code.asr[4],sep=',') }
  if (nrand!=0) { str.mod.ri <- paste(code.asr[1],code.asr[2],code.asr[3],code.asr[4],sep=',') }
  # mod.rc  
  code.asr[3] <- 'residual=~ar1(row):ar1(col)'
  code.asr[1] <- paste('mod.rc<-', code.t, sep='') 
  if (nrand==0) { str.mod.rc <- paste(code.asr[1],code.asr[3],code.asr[4],sep=',') }
  if (nrand!=0) { str.mod.rc <- paste(code.asr[1],code.asr[2],code.asr[3],code.asr[4],sep=',') }
  
  if (is.null(model)) { 
    
    eval(parse(text=str.mod.ii) )
    if (!mod.ii$converge) { eval(parse(text='mod.ii<-asreml::update.asreml(mod.ii)')) }
    eval(parse(text=str.mod.ic))
    if (!mod.ic$converge) { eval(parse(text='mod.ic<-asreml::update.asreml(mod.ic)')) }
    eval(parse(text=str.mod.ri))
    if (!mod.ri$converge) { eval(parse(text='mod.ri<-asreml::update.asreml(mod.ri)')) }
    eval(parse(text=str.mod.rc))
    if (!mod.rc$converge) { eval(parse(text='mod.rc<-asreml::update.asreml(mod.rc)')) }
    
    # Obtaining solutions/predictions for models
    if (type.gen=='fixed') {
      sol.ii=NULL; sol.ic=NULL; sol.ri=NULL; sol.rc=NULL; 
    ...
    if (is.null(check)) { checks=FALSE; ctable <- data.frame(table(df$gen,1:length(df$gen))) }
    if (!is.null(check)) { checks=TRUE; ctable <- data.frame(table(df$gen,df$check)) }
    ...
    gfit[1,6] <- gt$herit.PEV; gfit[1,7] <- gt$herit.VC; gfit[1,8] <- gt$Aopt; gfit[1,9] <- gt$Dopt 
    gt <- stats.spatial(object=mod.ic, checks=checks, solution=sol.ic, preds=pvals.ic, ctable=ctable)
    gfit[2,2] <- gt$n.vc; gfit[2,3] <- gt$logL; gfit[2,4] <- gt$aic; gfit[2,5] <- gt$bic; 
    gfit[2,6] <- gt$herit.PEV; gfit[2,7] <- gt$herit.VC; gfit[2,8] <- gt$Aopt; gfit[2,9] <- gt$Dopt
    gt <- stats.spatial(object=mod.ri, checks=checks, solution=sol.ri, preds=pvals.ri, ctable=ctable)
    gfit[3,2] <- gt$n.vc; gfit[3,3] <- gt$logL; gfit[3,4] <- gt$aic; gfit[3,5] <- gt$bic; 
    gfit[3,6] <- gt$herit.PEV; gfit[3,7] <- gt$herit.VC; gfit[3,8] <- gt$Aopt; gfit[3,9] <- gt$Dopt
    gt <- stats.spatial(object=mod.rc, checks=checks, solution=sol.rc, preds=pvals.rc, ctable=ctable)
    gfit[4,2] <- gt$n.vc; gfit[4,3] <- gt$logL; gfit[4,4] <- gt$aic; gfit[4,5] <- gt$bic; 
    gfit[4,6] <- gt$herit.PEV; gfit[4,7] <- gt$herit.VC; gfit[4,8] <- gt$Aopt; gfit[4,9] <- gt$Dopt 
    ...
    if (sum(is.na(gfit[1:4,2]))==0) { gfit[5,2] <- which.min(gfit[1:4,2]) }
    if (sum(is.na(gfit[1:4,3]))==0) { gfit[5,3] <- which.max(gfit[1:4,3]) }
    if (sum(is.na(gfit[1:4,4]))==0) { gfit[5,4] <- which.min(gfit[1:4,4]) }
    if (sum(is.na(gfit[1:4,5]))==0) { gfit[5,5] <- which.min(gfit[1:4,5]) }
    if (sum(is.na(gfit[1:4,6]))==0) { gfit[5,6] <- which.max(gfit[1:4,6]) }
    if (sum(is.na(gfit[1:4,7]))==0) { gfit[5,7] <- which.max(gfit[1:4,7]) }
    if (sum(is.na(gfit[1:4,8]))==0) { gfit[5,8] <- which.min(gfit[1:4,8]) }
    if (sum(is.na(gfit[1:4,9]))==0) { gfit[5,9] <- which.min(gfit[1:4,9]) }
...
pred_ran_aug <- function(model, gen = "gen"){
  ...
  gen_effects <- gen_effects %>% 
                 dplyr::mutate(solution = intercept + solution, 
...
pred_fix_aug <- function(model, gen = "gen"){
  ...
  gen_effects <- gen_effects %>% dplyr::mutate(lower = solution - 1.645*std.error,
                                               upper = solution + 1.645*std.error)
  return(gen_effects)
}
...
pred_check_aug <- function(model, gen = "gen"){
  ...
  check_fixed <- check_fixed %>% dplyr::mutate(gen = gen_names[1:dplyr::n()]) %>% dplyr::select(-z.ratio)
  ...
  test_random <- test_random %>% dplyr::mutate(gen = gen_names[1:dplyr::n()]) %>% dplyr::select(-z.ratio)
  ...
  gen_effects <- rbind(check_fixed, test_random) %>%
                 dplyr::mutate(solution = fix_coef[,1] + solution,
                               lower = solution - 1.645*std.error,
                               upper = solution + 1.645*std.error)
  return(gen_effects)
}
...
prediction_augment <- function(model, gen="gen"){
  ...
  if(opt_1){
    ...
    if(opt_2){ effects <-  pred_fix_aug(model, gen = gen)
    } else{ effects <- pred_ran_aug(model, gen = "gen") }
  }
  return(effects)
}
