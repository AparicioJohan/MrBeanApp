...
scoreChart <- function(model, x = "Comp1", y = "Comp2", plot = TRUE, gen_selected = NULL){
  ...
  if(plot){
    ...
    g2 <- sc %>% 
      ...
      echarts4r::e_visual_map(Score,scale = my_scale, color= c("#440154","#21908C","#FDE725") #c("#db4437","white","#4285f4")
      ) %>% 
      echarts4r::e_legend(show = FALSE) %>% 
      echarts4r::e_tooltip() %>% 
    ...
    if(!is.null(gen_selected)){
      ...
      if(! gen_selected %in% sc$gen) return(g2)
...
loadChart <- function(model, x = "fac_1" , y = "fac_2", plot = TRUE ,trial_selected = NULL){
  ...
  Env.means$estima  <- Env.means %>% dplyr::mutate(interc + coef) %>% dplyr::pull()
  ...
  if(dimen==1){
    ...
    Env.stats <- Env.stats %>%
      dplyr::mutate(Vg=round(Vg,2), 
                    fac_1=round(.data[[x]],2),
                    fac_2=0, 
                    Environment=round(Environment,2),
                    Vgs=round(e_scale(Vg),2)) %>% dplyr::arrange(Vg)
    y <- ""
  ...
  g5 <- Env.stats %>% 
    ...
    echarts4r::e_visual_map(Environment,scale = my_scale,color= c("#440154","#21908C","#FDE725")  #  c("#db4437","white","#4285f4")
    ) %>% 
    echarts4r::e_legend(show = FALSE) %>% 
    echarts4r::e_tooltip() %>% 
    echarts4r::e_tooltip(formatter = htmlwidgets::JS("
  ...
  if(!is.null(trial_selected)){
    ...
    if(! trial_selected %in% Env.stats$site) return(g5)
...
varExpChart <- function(model,  height = "65%",  top = "15%"){
  ...
  Vg <- Vg %>% round(.,3) %>% as.data.frame %>% tibble::rownames_to_column("site")
...
corrChart <- function(model, height = "65%", left = "20%"){
  ...
  g3 <- CC %>%
    ...
    echarts4r::e_tooltip() %>%  echarts4r::e_visual_map( color=c("#4285f4" ,"white","#db4437"),
                                    min = -1 ,
                                    max =  1 ,
                                    orient= 'horizontal',
                                    left= 'center',
...
latent_regress <- function(model, genotype , scale = T, text_labs = T, alpha_p = 0.5, return_data = F){
  ...
  L.star <- ASM$gammas[[1]]$`rotated loads` %>% data.frame() %>%  tibble::rownames_to_column("site")
