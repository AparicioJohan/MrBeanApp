...
mod_lme4_single_server <- function(input, output, session, data){
  ...
  alpha <- eventReactive( input$run, {
    ...
    tryCatch({ if(sum(is.na(dt$Response)) > 0.9*length(dt$Response ) ) stop("Missing data in the response")},
      error = function(e) {toastr_error(title = "Warning:", conditionMessage(e),position =  "bottom-right",progressBar = TRUE)})
